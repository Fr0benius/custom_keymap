...
KEYMAPS(
  ...
  [PRIMARY] = KEYMAP_STACKED
  (___,          Key_1, Key_2, Key_3, Key_4, Key_5, M(MACRO_TURN_LEDS_OFF),
   Key_Backtick, Key_Q, Key_W, Key_E, Key_R, Key_T, Key_Tab,
   Key_PageUp,   Key_A, Key_S, Key_D, Key_F, Key_G,
   Key_PageDown, Key_Z, Key_X, Key_C, Key_V, Key_B, Key_Escape,
   OSM(LeftControl), Key_Backspace, OSM(LeftGui), OSM(Key_LeftShift),
   OSL(NUMPAD),

   M(MACRO_ANY),      Key_6, Key_7, Key_8,     Key_9,         Key_0,         M(MACRO_SWITCH_MAC_MODE),
   OSM(Key_LeftGui),  Key_Y, Key_U, Key_I,     Key_O,          Key_P,         Key_Equals,
                      Key_H, Key_J, Key_K,     Key_L,         Key_Semicolon, Key_Quote,
   Key_Enter,         Key_N, Key_M, Key_Comma, Key_Period,    Key_Slash,     Key_Minus,
   OSM(Key_RightShift), OSM(Key_RightAlt), Key_Spacebar, OSM(Key_RightControl),
   OSL(FUNCTION)),
...
// Macro to turn off all LEDs
static void turnLEDsOffMacro(uint8_t key_state) {
  if (keyToggledOn(key_state)) {
    LEDOff.activate();
  }
}

//Macro to switch between Mac and Linux/Windows modes

static bool mac_mode = 1;

static void customModKeyMacro(uint8_t keyState) {
  if(keyIsPressed(keyState)) {
     kaleidoscope::hid::pressKey(mac_mode?Key_LeftGui:Key_LeftAlt, true);
  }
}
